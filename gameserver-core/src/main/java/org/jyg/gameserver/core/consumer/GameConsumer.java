...
//    private final Map<Integer , ResultHandler> waitCallBackMap = new HashMap<>();
    ...
    public GameConsumer() {
        ...
        this.instanceManager.putInstance(new ProxyFactoryManager());
...
//        }
    ...
    protected void onReceiveEvent(EventData<?> event) {
        ...
        try {
            ...
            if(costMill > 10){
                Logs.DEFAULT_LOGGER.error("{} event  cost more time {} data : {}",getClass().getSimpleName(), costMill, (event.getEvent() == null? "null" : event.getEvent().toString()));
            }
        } catch (Exception e) {
